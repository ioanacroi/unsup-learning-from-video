...
def read_and_decode(filename_queue):
  ...
  #image = tf.image.resize_images(image, dim_in)
...
def model(data, differences, hue, train=True):
	...
	with tf.variable_scope('center'):
		#center
                with tf.variable_scope("dilate1"):
                    kernel = _variable_with_weight_decay('dilate1',
                            shape=[3, 3, 512, 64*8],
                            stddev=5e-2,
                            wd=0.0) 
                    dil1 = tf.nn.convolution(input=pool1, filter=kernel, padding="SAME", dilation_rate=(1,1), name="dil1")
                    dil1 = tf.nn.relu(dil1)
                    endpoints['dil1'] = dil1

                with tf.variable_scope("dilate2"):
                    kernel = _variable_with_weight_decay('dilate2',
                            shape=[3, 3, 64*8, 64*8],
                            stddev=5e-2,
                            wd=0.0) 
                    dil = tf.nn.convolution(input=dil1, filter=kernel, padding="SAME", dilation_rate=(2,2), name="dil2")
                    dil = tf.nn.relu(dil)
                    endpoints['dil2'] = dil

                with tf.variable_scope("dilate4"):
                    kernel = _variable_with_weight_decay('dilate4',
                            shape=[3, 3, 64*8, 64*8],
                            stddev=5e-2,
                            wd=0.0) 
                    dil = tf.nn.convolution(input=dil, filter=kernel, padding="SAME", dilation_rate=(4,4), name="dil4")
                    dil = tf.nn.relu(dil)
                    endpoints['dil4'] = dil

                with tf.variable_scope("dilate8"):
                    kernel = _variable_with_weight_decay('dilate8',
                            shape=[3, 3, 64*8, 64*8],
                            stddev=5e-2,
                            wd=0.0) 
                    dil = tf.nn.convolution(input=dil, filter=kernel, padding="SAME", dilation_rate=(8,8), name="dil8")
                    dil = tf.nn.relu(dil)
                    endpoints['dil8'] = dil

                with tf.variable_scope("dilate16"):
                    kernel = _variable_with_weight_decay('dilate16',
                            shape=[3, 3, 64*8, 64*8],
                            stddev=5e-2,
                            wd=0.0) 
                    dil = tf.nn.convolution(input=dil, filter=kernel, padding="SAME", dilation_rate=(16,16), name="dil16")
                    dil = tf.nn.relu(dil)
                    endpoints['dil16'] = dil

                with tf.variable_scope("dilate32"):
                    kernel = _variable_with_weight_decay('dilate32',
                            shape=[3, 3, 64*8, 64*8],
                            stddev=5e-2,
                            wd=0.0) 
                    dil = tf.nn.convolution(input=dil, filter=kernel, padding="SAME", dilation_rate=(32,32), name="dil32")
                    dil = tf.nn.relu(dil)
                    endpoints['dil32'] = dil
	...
	with tf.variable_scope('up4'):
		#up4
		up4 = tf.image.resize_images(endpoints['dil32'], [16, 16], method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
...
def main(args):
    with tf.Graph().as_default():
        ...
        summary_writer = tf.summary.FileWriter("./summaries_unet_dilated/", sess.graph)
        ...
        for step in range(start_step, NUM_EPOCHS + 1):
          ...
          if step % 1000 == 0 and step!=0:
            saver.save(sess, "./checkpoints/unet_dilated/model_unet_dilated.ckpt-" + str(step))
